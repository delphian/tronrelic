...
export class TronLendingFetcher extends BaseMarketFetcher {
    ...
    constructor(context: IPluginContext) {
        super(context, { name: 'Tron Lending', guid: MARKET_GUID });
    }
    ...
    async pull(): Promise<MarketSnapshot | null> {
        ...
        const infoResponse = await executeWithRetry(
            () =>
                this.context.http.get<TronLendingInfoResponse>(this.config.endpoints.info, {
        ...
        try {
            const rateResponse = await executeWithRetry(
                () =>
                    this.context.http.get<TronLendingRateResponse>(this.config.endpoints.rate, {
                        timeout: this.timeoutMs
                    }),
                {
                    logger: this.context.logger,
            ...
            this.context.logger.warn({ error, fetcher: this.name }, 'Failed to fetch rate data, continuing with info only');
        }
        ...
        if (infoFields.length >= 4) {
            ...
            if (this.context.chainParameters) {
                availableEnergy = this.context.chainParameters.getEnergyFromTRX(availableTrx) || 0;
                totalEnergy = this.context.chainParameters.getEnergyFromTRX(totalTrx) || 0;
            }
        }
