...
export class MeFreeNetFetcher extends BaseMarketFetcher {
  ...
  constructor(context: IPluginContext) {
    super(context, { name: 'MeFree.Net', guid: MARKET_GUID });
  }
  ...
  async pull(): Promise<MarketSnapshot | null> {
    ...
    const response = await executeWithRetry(
      () =>
        this.context.http.get<MeFreeResponse>(this.config.endpoints.info, {
    ...
    const [chineseHtml, englishHtml] = await Promise.all([
      executeWithRetry(
        () =>
          this.context.http.get<string>('https://mefree.net/', {
            timeout: this.timeoutMs,
            responseType: 'text'
          }),
        {
      ...
      executeWithRetry(
        () =>
          this.context.http.get<string>('https://mefree.net/en/', {
            timeout: this.timeoutMs,
            responseType: 'text'
          }),
        {
