...
export class FeeeIoFetcher extends BaseMarketFetcher {
  ...
  constructor(context: IPluginContext) {
    super(context, { name: 'Feee.io', guid: MARKET_GUID });
  }
  ...
  private async fetchTrades() {
    ...
    while (hasMore) {
      const response = await executeWithRetry(
        () =>
          this.context.http.get<FeeeTradesResponse>(this.config.endpoints.trades, {
  ...
  async pull(): Promise<MarketSnapshot | null> {
    const [energyResponse, configResponse, trades] = await Promise.all([
      executeWithRetry(
        () =>
          this.context.http.get<FeeeEnergyResponse>(this.config.endpoints.energy, {
            timeout: this.timeoutMs
          }),
        {
          logger: this.context.logger,
      ...
      executeWithRetry(
        () =>
          this.context.http.get<FeeeConfigResponse>(this.config.endpoints.config, {
            timeout: this.timeoutMs
          }),
        {
          logger: this.context.logger,
    ...
    const fees = [
      {
        ...
        apy: this.context.chainParameters?.getAPY
          ? this.context.chainParameters.getAPY(100_000, priceConfig.default_sun_1h, 1 / 24)
          : undefined
      },
      {
        minutes: 60 * 24,
        ...
        apy: this.context.chainParameters?.getAPY
          ? this.context.chainParameters.getAPY(100_000, priceConfig.default_sun_1d, 1)
          : undefined
      },
      {
        minutes: 60 * 24 * 3,
        ...
        apy: this.context.chainParameters?.getAPY
          ? this.context.chainParameters.getAPY(100_000, priceConfig.default_sun, 3)
          : undefined
      }
    ];
