...
export class TronPulseFetcher extends BaseMarketFetcher {
  ...
  constructor(context: IPluginContext) {
    super(context, { name: 'Tron Pulse', guid: MARKET_GUID });
  }
  ...
  private async fetchOrders(): Promise<OrderSnapshot[]> {
    const response = await executeWithRetry(
      () =>
        this.context.http.get<OrdersResponse>(this.config.endpoints.orders, {
  ...
  async pull(): Promise<MarketSnapshot | null> {
    ...
    const results = await Promise.allSettled([
      executeWithRetry(
        () =>
          this.context.http.get<LiquidityResponse>(this.config.endpoints.liquidity, {
            timeout: this.timeoutMs,
            headers
          }),
        {
      ...
      executeWithRetry(
        () =>
          this.context.http.get<MarketSettingsResponse>(this.config.endpoints.settings, {
            timeout: this.timeoutMs,
            headers
          }),
        {
    ...
    const fees = (settings?.energy_duration ?? [])
      .map(duration => {
        ...
        const apyValue = this.context.chainParameters?.getAPY?.(1_000_000, duration.energy_min_price, minutes / (60 * 24));
