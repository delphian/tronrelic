...
export class TronEnergyMarketFetcher extends BaseMarketFetcher {
  ...
  constructor(context: IPluginContext) {
    super(context, { name: 'Tron Energy Market', guid: MARKET_GUID });
  }
  ...
  async pull(): Promise<MarketSnapshot | null> {
    const [infoResponse, ordersResponse] = await Promise.all([
      executeWithRetry(
        () =>
          this.context.http.get<TronEnergyInfoResponse>(this.config.endpoints.info, {
      ...
      executeWithRetry(
        () =>
          this.context.http.get<TronEnergyOrderResponse>(this.config.endpoints.orders, {
            timeout: this.timeoutMs
          }),
        {
          logger: this.context.logger,
    ...
    const fees = (info.price?.openEnergy ?? [])
      .map(entry => {
        ...
        const apy = this.context.chainParameters?.getAPY
          ? this.context.chainParameters.getAPY(100_000, entry.basePrice, entry.minDuration / (24 * 60))
          : undefined;
