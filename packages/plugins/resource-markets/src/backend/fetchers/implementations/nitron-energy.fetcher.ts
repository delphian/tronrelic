...
function extractOrders(html: string, context: IPluginContext): OrderSnapshot[] {
...
export class NitronEnergyFetcher extends BaseMarketFetcher {
  ...
  constructor(context: IPluginContext) {
    super(context, { name: 'NiTron Energy', guid: MARKET_GUID });
  }
  ...
  async pull(): Promise<MarketSnapshot | null> {
    const [infoResponse, homeResponse] = await Promise.all([
      executeWithRetry(
        () =>
          this.context.http.get<NitronInfoResponse>(this.config.endpoints.info, {
    ...
    const orders = extractOrders(html, this.context);
        ...
        ? [
            {
              minutes: 60 * 24,
              sun: fee24h,
              apy: this.context.chainParameters?.getAPY?.(1_000_000, fee24h, 1)
            }
          ]
        : undefined,
      orders,
