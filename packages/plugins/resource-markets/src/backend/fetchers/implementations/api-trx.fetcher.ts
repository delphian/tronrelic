...
export class ApiTrxFetcher extends BaseMarketFetcher {
  ...
  constructor(context: IPluginContext) {
    super(context, { name: 'Api TRX', guid: MARKET_GUID });
  }

  async pull(): Promise<MarketSnapshot | null> {
    const response = await executeWithRetry(
      () =>
        this.context.http.get<string>(this.config.endpoints.price, {
    ...
    const fees = extractFees(html).map(fee => ({
      minutes: fee.minutes,
      sun: fee.sun,
      apy: this.context.chainParameters?.getAPY?.(1_000_000, fee.sun, fee.minutes / (60 * 24))
    }));
