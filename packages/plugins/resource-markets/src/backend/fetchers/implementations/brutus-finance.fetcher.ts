...
export class BrutusFinanceFetcher extends BaseMarketFetcher {
  ...
  constructor(context: IPluginContext) {
    super(context, { name: 'Brutus Finance', guid: MARKET_GUID });
  }
  ...
  async pull(): Promise<MarketSnapshot | null> {
    ...
    const [availabilityResponse, pricingResponse] = await Promise.all([
      executeWithRetry(
        () =>
          this.context.http.get<AvailabilityResponse>(this.config.endpoints.availability, {
      ...
      executeWithRetry(
        () =>
          this.context.http.get<PricingResponse>(this.config.endpoints.pricing, {
            timeout: this.timeoutMs
          }),
        {
          logger: this.context.logger,
