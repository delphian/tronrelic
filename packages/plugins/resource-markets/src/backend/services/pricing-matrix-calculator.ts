...
export async function computePricingDetail(
    ...
    // Calculate USDT transfer costs from site fees (pass usdtService)
    const usdtTransferCosts = await calculateUsdtTransferCostsForAllDurations(usdtService, fees);
    const minUsdtTransferCost = await getMinUsdtTransferCost(usdtService, fees);
