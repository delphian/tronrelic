...
15. **Remaining module inits in this order:** Logs, Pages, Widgets, Scheduler, Identity, Traffic, AddressLabels, Tools, then ClickHouseAccounts after Syndication (Traffic receives `scheduler` and `clickhouse` on top of `sharedDeps`; ClickHouseAccounts receives `clickhouse` twice, as the shared connection and as its `IClickHouseAccountConnector`, plus `scheduler`)

**`bootstrapRun()`** — in this order:

Database, ClickHouse, Menu, Logs, Pages, Widgets, Traffic, Identity, AddressLabels, Tools, ClickHouseAccounts, **Scheduler last** (so it doesn't tick before its peers are integrated). Traffic runs before Identity so its `/api/admin/users/{traffic,analytics}` routers mount ahead of Identity's `/api/admin/users` accounts `/:id` catch-all.
