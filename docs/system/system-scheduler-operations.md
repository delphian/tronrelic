...
Nine jobs registered in `src/backend/modules/scheduler/jobs/core-jobs.ts`:
...
| `blockchain:prune-transactions` | `* * * * *` | Drop transactions older than 4 days, at most 10 batches of about 2,000 a run with a one-second pause between batches | `transactions` grows unbounded and block commits slow as its indexes outgrow MongoDB's cache |
| `blockchain:prune` | `0 * * * *` | Drop blocks (32d) past retention | `blocks` collection grows unbounded |
