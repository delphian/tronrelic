...
| `http.sitemapEntries` | `http.api` | `waterfall` | 200 | Fired while `GET /api/sitemap-data` assembles the sitemap. Threads `ReadonlyArray<ISitemapEntry>` (`{ path, lastModified?, changeFrequency?, priority? }`), seeded `[]`; core absolutizes each root-relative `path`. Lets a plugin owning many crawlable resources (blog posts, forum threads) contribute per-resource URLs core cannot enumerate without reaching into the plugin's storage. `trp-blog` registers here to list its published posts. |
