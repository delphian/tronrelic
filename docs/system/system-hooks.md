...
| `ai.toolResult` | `ai.tool` | `waterfall` | 150 | Fired after a governed AI tool's handler returns, before the provenance wrap and untrusted-content screen. Threads the raw result value (`IAiToolInvokeContext`); a handler alters it (redact / reshape / summarize) or throws `HookAbortError` to withhold it from the model. The raw result is still digested into the audit record. |
...
| `http.sitemapEntries` | `http.api` | `waterfall` | 200 | Fired while `GET /api/sitemap-data` assembles the sitemap. Threads `ReadonlyArray<ISitemapEntry>` (`{ path, lastModified?, changeFrequency?, priority? }`), seeded `[]`; core absolutizes each root-relative `path`. Lets a plugin owning many crawlable resources (blog posts, forum threads) contribute per-resource URLs core cannot enumerate without reaching into the plugin's storage. `trp-blog` is the intended first consumer, listing its published posts. |
...
| `content.beforeCreate` | `content.lifecycle` | `series` | 20 | Fired by the core content service before it creates a managed content item. Carries `IContentWriteContext` (`operation`, `typeId`, the new `id`, `actor`, `input`). Throw `HookAbortError` to refuse the create; the service reports the `vetoed` error code. See [system-content.md](./system-content.md). |
| `content.beforeUpdate` | `content.lifecycle` | `series` | 30 | Same context, fired before a change to a managed item. Throw `HookAbortError` to refuse it. |
| `content.beforeDelete` | `content.lifecycle` | `series` | 40 | Same context without `input`, fired before a soft delete. Throw `HookAbortError` to refuse it. |
| `content.beforeRestore` | `content.lifecycle` | `series` | 50 | Same context without `input`, fired before a soft-deleted item is restored. Throw `HookAbortError` to keep it deleted. |
| `content.published` | `content.lifecycle` | `observer` | 100 | Fired when an approved curation item is committed and its canonical content is live — one firing per approved item, from inside curation's decision commit. Carries `IContentPublishedContext` (`typeId` + `ref` + decision-time `descriptor`) so a subscriber can load the full record and react. Fires only after a successful approved-status `applyEdit` commit; a rejected decision or a commit that throws does not fire it; cannot change the outcome. |
