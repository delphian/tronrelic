...
A content type — an item the platform stores and publishes — must be [managed content](./system-content.md) and does not implement `ICurationType`; core registers its curation binding for it (see [Managed Content](#managed-content) below). The rest of this section describes `ICurationType` itself, which a provider implements to hold an effect that is not content, and which the older content types below still use until they migrate.
...
| `decisionStatus` | Required. The originator's status word per decision: `{ approved?, rejected }`. `rejected` is required — rejection is intrinsic to review, and nothing else carries it. Declare `approved` when approval commits the type's own record synchronously (publish it, release it, mark it); omit it only for a type with no synchronous approval effect that relies entirely on routed sinks |

Both `decisionStatus` and `applyEdit` are required on every curation type — the coherence guard rejects a type missing either at registration. There is no other commit seam: core writes the declared word through `applyEdit({ status })`, and no imperative post-decision callback exists. A type that needs a side effect on approval routes it through a [content-router publish sink](./system-content-routing.md#selecting-the-mandated-subset-at-the-gate), not through the binding. Core does fire one *observer* seam at the commit — `content.published` (see [system-hooks.md](./system-hooks.md)), when an approval synchronously commits canonical content (the type declared `decisionStatus.approved`) — a fan-out notification for downstream reactors (announce, index, cache) that cannot change the decision. It is a notification, not a commit callback.
...
**Approval has two independent effects; a type may use either or both.** `decisionStatus.approved` commits the type's *own* record synchronously through `applyEdit`, the instant the decision commits — `blog:post` renders its draft and flips it live in place, so the canonical post exists before any downstream reactor (e.g. a syndicated announcement fired off `content.published`) can reference it. Curator-selected [publish sinks](./system-content-routing.md#selecting-the-mandated-subset-at-the-gate) additionally deliver the approved content to *external* surfaces (a channel, another site) through the durable syndication outbox. The two coexist safely: if a curator routes a self-publishing type to a sink that targets its own surface, the owning store makes it a no-op — the `blog:site` sink finds the slug already published and settles without duplicating. Declare `approved` when approval commits something of the type's own (publish a post, release an AI action, apply a moderation decision); omit it only for a type with no synchronous approval effect that relies entirely on routed sinks. When a self-published record must carry full fidelity to its own surface, pass the enrichment (a reserved slug, tags) through the descriptor's governed `fields`.

A `publishesToSinks` type with any eligible sink must have at least one sink selected on approval: curation blocks an empty-selection approval at the service (mirrored by the picker's disabled Approve button), so a decision can never record while publishing nowhere. A type with zero eligible sinks — a classic type, or a sinks type whose transports are all disabled — approves to nowhere, the only available outcome, so the guard never deadlocks a queue.

### Managed Content

A [managed content](./system-content.md) type does not implement `ICurationType` at all. The core content service generates the binding for it — `decisionStatus: { approved: 'approved', rejected: 'rejected' }`, `publishesToSinks: false`, an `applyEdit` that forwards the decision back to the content service — and registers it here whenever curation is available. Core decides when a managed item enters the queue (a non-curator changed a field the type declares as reviewed), and on approval the content service asks the author to promote its working version, so visitors keep seeing the last approved version until then. Managed items are read-only in this queue and are edited in their own editor, so every change passes through the core write path. `core:page` is the first managed type. `blog:post`, `memo-tracker:curated-memo`, and `core:social-post` still implement `ICurationType` directly and must migrate; do not model new content types on them.
...
// is the reference type: a sink-routable post the curator fans out.
context.services.watch<ICurationService>('curation', {
    onAvailable: (curation) => curation.registerType({
        typeId: 'core:social-post',
        label: 'Social Post',
        publishesToSinks: true,                                     // curator picks publish sinks on approval
...
- [AI Tools Module README](../../src/backend/modules/ai-tools/README.md) — the governor that verifies a tool's `curationTypeId` against the curation service, plus `core:social-post` (the reference `publishesToSinks` type) and the `propose-social-post` tool that holds into it
