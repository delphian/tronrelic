...
Core defines the tool contract, the capability vocabulary, and the registry, governor, policy, and audit. An AI **provider plugin** is only a transport: it implements the `IAiProvider` execution contract (`query` / `ask` / `queryStream` / `cancel` / `listModels` / `listActiveServerTools`), formats tool declarations for its vendor's API, runs the agentic loop, and routes each tool call back through core. A provider that hosts its own server-side tools carries one extra obligation core cannot enforce for it: honour the `hosted:` half of the run's `toolAllowlist` by not declaring an ungranted tool, since core has no other way to stop a call the vendor executes. `trp-ai-assistant` is the Anthropic provider; an OpenAI or Google provider would be a separate plugin. Tools are provider-neutral and must never import or assume a specific provider.
