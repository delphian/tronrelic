...
| Configuration | Sign-in button image | `AuthButtonImageSection` | GET/PATCH `/config/system` | Choose an image with the file picker to replace the sign-in button; the `core:auth-button` widget reads it server-side through its data fetcher, so visitors see a change within the widget cache's five seconds. Choosing needs an enabled files provider (`trp-files` by default) |
