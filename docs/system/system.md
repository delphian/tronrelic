...
| Blockchain Sync | Two loops. Preparing pulls blocks from TronGrid one at a time with a 200ms delay between calls and enriches each transaction with USD and energy figures, never waiting on a clock and never writing. Committing holds a lead of prepared blocks in `BlockEmitter` and releases them at the chain's own cadence — writing the block, advancing the cursor, notifying observers, and broadcasting `block:new` together. Because a block does not exist until its slot arrives, every read surface reports the same height. With receipts on, each transaction's event logs are decoded into token transfers and delivered to event observers by signature. | [system-blockchain-sync-architecture.md](./system-blockchain-sync-architecture.md), [system-blockchain-contract-events.md](./system-blockchain-contract-events.md) |
| Scheduler | Runs eight built-in recurring jobs that operators can toggle at runtime. `blockchain:sync`, `chain-parameters:fetch`, and `usdt-parameters:fetch` are the critical ones. `blockchain:sync` is coupled to the sync lock's TTL and the two must move together. `ENABLE_SCHEDULER=false` disables all of them. | [system-scheduler-operations.md](./system-scheduler-operations.md) |
...
| Pages | A markdown content management system built from `IPageService`, with rendered output cached in Redis and a list of routes it may not claim. Pages are managed content (`core:page`), so edits from non-curators are reviewed and deletion is soft. | [Pages README](../../src/backend/modules/pages/README.md) |
...
| ClickHouse Accounts | Creates and maintains ClickHouse users, such as the AI agent's read-only `tron` account, whose per-query limits and hourly quota the server enforces. Admins tune limits up to code-set ceilings on the ClickHouse tab of `/system/system`, and every change is audited. Publishes `'clickhouse-accounts'`, whose `reader(id)` hands code a connection bound to an account. | [ClickHouse Accounts README](../../src/backend/modules/clickhouse-accounts/README.md) |
...
| Providers | The vendor registry, runtime configuration, and clients for external data vendors (TronScan, CoinGecko, GeckoTerminal, TronGrid). A vendor is declared once with a descriptor and a test; a generic card on `/system/system` → Configuration edits it, and a consumer asks the registry for vendors by capability such as `price-history`. | [Providers README](../../src/backend/modules/providers/README.md) |
| Price History | Keeps a local daily USD price series for TRX and TRC20 tokens in ClickHouse, routed across the configured price vendors in an operator-set order per asset class. Publishes `'price-history'`. | [Price History README](../../src/backend/modules/price-history/README.md) |
...
| Address Tags | The single authority for text tags on TRON wallet addresses, with per-tag provenance: scheduled jobs ingest the OFAC sanctions list and Tether's freeze feed as cited, soft-withdrawable machine assertions (`ofac:sdn`, `usdt:frozen`), and an admin can screen one address through Chainalysis on demand. Publishes `'address-tags'`. Reads require login, changes require admin, and `/system/address-tags` is the interface. | [Address Tags README](../../src/backend/modules/address-tags/README.md) |
...
| Managed Content | The core content service every create, read, update, delete, and restore of managed content passes through. Items extend `IContent`; core stores the shared fields in `content_items`, runs the `content.before*` veto hooks, holds non-curator changes to reviewed fields in curation, and serves visitors the last approved version. Publishes `'content'`. | [system-content.md](./system-content.md) |
...
| [system-chain-query-tools.md](./system-chain-query-tools.md) | The five read-only AI tools that walk the `tron` database as the `ai-agent` account: wallet profile, transfers, counterparties, multi-hop tracing, and token activity, with the response envelope they share |
