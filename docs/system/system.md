...
| Traffic | Analytics that avoid identifying individuals: events land in the ClickHouse `traffic_events` table, the `tronrelic_tid` and `tronrelic_ref` cookies carry visit context, and traffic is classified by bot likelihood and geography. | [Traffic README](../../src/backend/modules/traffic/README.md) |
