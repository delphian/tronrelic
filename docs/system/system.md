...
| Blockchain Sync | Two loops. Ingestion pulls blocks from TronGrid one at a time with a 200ms delay between calls, enriches each transaction with USD and energy figures, and dispatches to observers asynchronously, never waiting on a clock. Broadcast holds a lead of finished blocks in `BlockEmitter` and releases them at the chain's own cadence, so an upstream hiccup is covered rather than shown. | [system-blockchain-sync-architecture.md](./system-blockchain-sync-architecture.md) |
