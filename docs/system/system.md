...
| Scheduler | Runs nine built-in recurring jobs that operators can toggle at runtime. `blockchain:sync`, `chain-parameters:fetch`, and `usdt-parameters:fetch` are the critical ones. `blockchain:sync` is coupled to the sync lock's TTL and the two must move together. `ENABLE_SCHEDULER=false` disables all of them. | [system-scheduler-operations.md](./system-scheduler-operations.md) |
