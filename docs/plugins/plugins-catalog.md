...
| trp-onchain-typologies | Models a named on-chain behaviour (a typology) as a composition of declared slots run over a shared substrate; captures what a composition's vocabulary cannot yet name under an `x:` prefix, so a later detector fix promotes stored history instead of only reaching new traffic. Ships pooled resource rental and pool membership; admin-only workbench, no public pages; publishes `onchain-typologies`. | [README](../../src/plugins/trp-onchain-typologies/README.md) |
