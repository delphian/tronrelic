...
**The auth gate runs before your `middleware`.** On a gated route — `requiresAuth`, `requiresAdmin`, or any route under `/system/**` — `PluginApiService` mounts `requireLogin`/`requireAdmin` ahead of everything in the `middleware` array, so an unauthorized caller is turned away before your middleware sees the request. Rely on this whenever your middleware consumes a resource on the caller's behalf: a body parser that buffers an upload, a request to a paid third-party API, an expensive cache warm. The corollary is that middleware on an **ungated** route runs for anonymous callers by definition, so a public route's own middleware must be cheap and self-limiting — that is exactly the right place for a rate limiter.
