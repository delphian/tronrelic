...
Keeping everything in one repository is a development convenience. It does not make another plugin's code available to depend on. Importing another plugin's source or types because the two directories sit side by side is a bug even when it compiles here, because it breaks as soon as that plugin is installed on its own. To check your work: if a plugin would not build against only the dependencies it declares, it is wrong.
...
**A plugin publishes exactly one registry name**, and everything a programmatic caller may do with that plugin is reachable from the object behind it. Where the plugin offers several distinct responsibilities, that object is a root authority whose accessors return the focused services — reading, administration, diagnostics — rather than one interface carrying every method. See [plugins-service-registry.md](./plugins-service-registry.md).
