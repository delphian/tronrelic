...
**Content.** A content type — an item the platform stores and publishes, such as a post, article, or draft — must be managed content. Implement `IManagedContentType` and register it on the core `'content'` service (`IContentService.registerType`) during `init()`, calling the returned disposer in `disable()`. Store only the type's own fields in the plugin's collection, and send every create, read, update, delete, and restore through `IContentService` rather than calling your storage methods directly. Core then runs the veto hooks, records who acted, holds non-curator changes for review, and serves visitors the last approved version. Do not implement `ICurationType` for a content type. See [system-content.md](../system/system-content.md).
