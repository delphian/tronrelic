...
# Example:
./scripts/droplet-deploy.sh <PROD_DROPLET_IP>
```
...
- Domain DNS A record pointing dev.tronrelic.com to droplet IP (<OLD_DEV_DROPLET_IP>)
- GitHub Personal Access Token with `read:packages` scope
...
**What the script does:**
1. **Verifies SSH connection** to development server (<OLD_DEV_DROPLET_IP>)
...
Application URLs:
  Frontend: http://<OLD_DEV_DROPLET_IP>/
  Backend:  http://<OLD_DEV_DROPLET_IP>/api
  System:   http://<OLD_DEV_DROPLET_IP>/system

View logs with:
  ssh root@<OLD_DEV_DROPLET_IP> 'cd /opt/tronrelic-dev && docker compose logs -f'
```
...
2. **Automatic Deployment:**
   - SSH to dev.tronrelic.com (<OLD_DEV_DROPLET_IP>)
...
ssh root@<OLD_DEV_DROPLET_IP> 'cd /opt/tronrelic-dev && docker compose logs --tail=100 -f'
```
