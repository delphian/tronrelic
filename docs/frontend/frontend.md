...
The log service name comes from the logger's bindings. A module that logs through `logger.child({ module: '<id>' })` records its entries under `tronrelic:<id>`, and a plugin's injected logger records under `plugin:<id>`. A child logger that sets its own `service` binding records under that name instead and falls outside the Logs tab, so do not set one in a component that has a Logs tab. A module must also pass its logger when it registers a scheduler job, as `scheduler.register(name, cron, fn, { logger })`, or the scheduler's own failure entry for that job is filed under `tronrelic` and never reaches the module's Logs tab. A plugin gets this without doing anything.
...
Order matters, because an outer provider has to be available to the ones inside it: Redux comes first so that every component can reach the store, then `SessionProvider`, then the toast and modal providers so that plugins can call their hooks.

The toast and modal providers supply their context but do not draw anything. The markup comes from `<ToastViewport />` and `<ModalViewport />`, mounted as the last children inside every provider. A portal renders from the tree position of the component that created it, so markup drawn by a provider itself can only reach the providers above that provider — which is how a toast body calling `useModal`, or a modal body calling `useToast`, ended up with no context at all. Keep both viewports at the bottom of the stack. See [react.md](./react/react.md#provider-composition).
