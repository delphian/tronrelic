...
| `<Input>` | Text input with focus ring | `variant="default\|ghost"`, `size="xs\|sm\|md\|lg"` + `InputHTMLAttributes` except native `size` | [Input](../../../src/frontend/components/ui/Input/) |
...
| `<Select>` | Themed native `<select>` with shared input tokens and a Lucide chevron; intrinsic width by default (pass a `width:100%` class to fill) | `variant="default\|ghost"`, `size="xs\|sm\|md\|lg"` + `SelectHTMLAttributes` except native `size` | [Select](../../../src/frontend/components/ui/Select/) |
...
| `<Textarea>` | Themed multiline field matching the Input/Select tokens; `forwardRef` for imperative focus | `variant="default\|ghost"`, `size="xs\|sm\|md\|lg"` + all `TextareaHTMLAttributes` | [Textarea](../../../src/frontend/components/ui/Textarea/) |
