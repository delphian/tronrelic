...
| `<AddressSelector>` | The canonical control for *choosing* a TRON address (as `<TronAddress>` is for rendering one). Accepts a pasted address, or offers a typeahead over tagged addresses matching address or tag text, each shown with its tags. Self-contained; emits validated base58 only. Suggestions are login-gated and degrade to a plain input when absent | `value` (address or null), `onChange`, `disabled`, `placeholder`, `aria-label`, `limit`, `id`, `aria-describedby`, `aria-required`. The last three are forwarded to whichever control the current state renders, so `<Field label="…">` can label it like any other control | [AddressSelector](../../../src/frontend/components/ui/AddressSelector/) |
| `<Badge>` | Inline status/label pill. `size` steps the padding around the label and nothing else — the badge's type already sits at the design system's 0.72rem floor, so a dense list gets a tighter pill rather than smaller words. Reach for `sm` or below in a narrow rail, a sidebar, or a table cell, where the default pill takes more width than the label earns | `tone="neutral\|info\|success\|warning\|danger"`, `size="xs\|sm\|md\|lg"`, `showLiveIndicator` | [Badge](../../../src/frontend/components/ui/Badge/) |
...
| `<Input>` | Text input with focus ring. `size` moves padding *and* type together, on a ladder `<Select>` and `<Textarea>` share, so a dense control reads as one step down rather than as a full-size field squeezed into less room; the `xs` step holds its type at the 0.72rem legibility floor. `invalid` turns the border and focus ring danger and sets `aria-invalid`, without changing the rendered structure — still a bare `<input>`, so a control that is a flex item stays one. Pair it with `<Field>` to say *why* the value was refused | `variant="default\|ghost"`, `size="xs\|sm\|md\|lg"`, `invalid` + `InputHTMLAttributes` except native `size` | [Input](../../../src/frontend/components/ui/Input/) |
...
| `<TronAddress>` | **The canonical way to render a TRON wallet address.** Compact monospace chip — truncated `first 4…last 4`, full value plus any address tags in the tooltip — carrying three slim affordances: copy, a "forward to a public tool" menu (with the tag editor for admins), and a Tronscan out-link. Renders synchronously from `address`, so it is SSR-safe; pass a pre-resolved `label` to show a name instead of the truncation (it resolves none itself). Never hand-truncate an address or hand-build a Tronscan anchor. For a smart contract use `<TronContractAddress>` | `address`, `label`, `copy`, `tools`, `explorer` (affordances default on), `className` | [TronAddress](../../../src/frontend/components/ui/TronAddress/) |
| `<TronContractAddress>` | **The canonical way to render a smart contract address**, as `<TronAddress>` is for a wallet. The same chip with the same props — tags, warning marker, copy, and the tag editor for admins — but the out-link opens Tronscan's contract page, and the tools menu offers only tool pages that accept a contract (today the Address Converter). Wallet-only tools such as the Signature Verifier and Address Origins are left out because they come back empty for a contract. Use it for token contracts, DEX routers, and any observer's `contract_address` | `address`, `label` (e.g. a token symbol), `copy`, `tools`, `explorer` (affordances default on), `className` | [TronContractAddress](../../../src/frontend/components/ui/TronContractAddress/) |
...
| `ToastProvider` | Viewport toast queue with variants, auto-dismiss, actions. `titleHref` links the title to the page that shows the event in full; `description` takes rendered content, so a value keeps the component it is shown with everywhere else (an address stays a `<TronAddress>` rather than becoming a bare string) | `useToast()` → `{ push, dismiss }` | [ToastProvider](../../../src/frontend/components/ui/ToastProvider/) |
