...
| `<Select>` | Themed native `<select>` with shared input tokens and a Lucide chevron; intrinsic width by default (pass a `width:100%` class to fill) | `variant="default\|ghost"` + all `SelectHTMLAttributes` | [Select](../../../src/frontend/components/ui/Select/) |
...
| `<Textarea>` | Themed multiline field matching the Input/Select tokens; `forwardRef` for imperative focus | `variant="default\|ghost"` + all `TextareaHTMLAttributes` | [Textarea](../../../src/frontend/components/ui/Textarea/) |
