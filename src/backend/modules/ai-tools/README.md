...
| POST | `/trifecta/preview` | Same status scoped to a hypothetical `toolAllowlist` (body) — backs the saved-prompt editor's per-run badge. Filters both the governed registry tools and the `hosted:`-prefixed provider tools to the selection; secret variables still fold in. 400 on a malformed allowlist |
