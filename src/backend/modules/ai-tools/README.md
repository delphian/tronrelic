...
| Hook seams | Declares `ai.toolInvoke` (series, veto/hold) and `ai.toolInvoked` (observer, audit fan-out); subscribes as `'core'` to `content.published` to enqueue hook-bound saved prompts |
