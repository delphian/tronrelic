...
| `getWalletFlow(address, granularity)` | Just the inflow/outflow buckets at `'month'`/`'week'`/`'day'` resolution — split out so the profile chart's precision selector re-buckets without re-running the whole summary (the summary still carries the monthly view for the initial render) |
...
| GET | `/api/account-history/me/wallets/:address/flow` | `{ flow }` — inflow/outflow buckets for one owned wallet at the `?granularity=` resolution (`month`/`week`/`day`); backs the profile chart's precision selector |
