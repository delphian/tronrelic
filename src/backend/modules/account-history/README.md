...
The provider walks **three** TronGrid account endpoints, because none alone is complete:
...
- `/v1/accounts/{addr}/internal-transactions` (source `internal`) — TVM value moves a contract performs mid-execution (a contract paying TRX to the account), invisible to the two transaction endpoints. These are value legs, not transactions, so they land in `account_value_transfers` only — see [Storage](#storage).
...
**Token value legs come from a fourth source: per-transaction events.** The `trc20` endpoint surfaces a token transfer but omits its protocol `log_index`, so a value leg built from it could only use an empty `leg_key` — and two distinct same-token transfers in one transaction would then collapse under the `account_value_transfers` natural key. The authoritative token leg is instead sourced from `/v1/transactions/{txId}/events` (`provider.fetchTokenTransferLegs`), whose `event_index` *is* the `log_index`. This is not a fingerprint-paged account walk: it is driven per token-bearing transaction the `trc20` walk discovers (inbound and outbound both surface there), so it needs no fourth cursor and costs one events call per such transaction, routed through the shared rate limiter. Decimals — absent from the events payload — are carried over from the matching `trc20` row's token metadata.
