...
A fetch that fails outright, with every vendor asked returning an error, gets a separate and shorter schedule. The cursor's `failedAttempts` counts consecutive failures, and `nextAttemptAt` holds the asset for 15 minutes after the first, doubling up to 6 hours. The day bounds stay where they were, and both fields clear as soon as a fetch gets any answer, whether priced or not. Before this schedule existed, a failing asset stayed due and was the least recently updated, so every five-minute tick asked the same refusing vendors again and no other asset's deep walk could advance. The Coverage tab shows a held asset as **Vendor errors** with its retry time, and each failure is logged at `error` level with the attempt count. Deployments that ran the TRX-only vendor recorded tokens as seeded-but-empty; migration `002_unpark_empty_token_cursors` clears those so the token vendors get to see them. TRX cursors and stored TRX rows are untouched by any of this.
