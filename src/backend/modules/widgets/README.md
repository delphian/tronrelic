...
| `widget-types/core-widget-types.ts` | Core widget-type catalog, built by `buildCoreWidgetTypeDescriptors(deps)` (`core:raw-html`, `core:world-clocks`, `core:layout-group`, `core:block-ticker`, `core:network-activity`, `core:auth-button`); `WidgetsModule.run()` registers each as `'core'`-owned. Frontend renderers live in `components/widgets/widgets.core.ts` |
...
**Zones** live in `zones/descriptors.ts`. The `Site top` zone (id `site-top`, `host: 'site'`) is the first element inside `<body>`, wrapped in the page's `<header>` landmark, and is where the site header is built: there is no fixed header component, only the core Site logo, Main menu, and Sign-in button widgets an operator places here. The `Site Header` zone (id `ticker-after`, `host: 'site'`) renders directly below `site-top` and is where the block ticker is seeded; the `footer` zone (`host: 'site'`) renders below `<main>` inside a semantic `<footer>`. All three reach every route the root layout serves, admin pages included. Adding a zone requires a matching `<WidgetZone>` call site in a layout; descriptor and render site move together.

`site-top` deliberately applies no width constraint or styling of its own, so it can hold either a full-bleed strip or content aligned to the page. To reproduce the old header, place the logo, menu, and sign-in button in a `core:layout-group` laid out as a row, give the menu a width weight so it takes the spare space, and set the zone's `customCss` to a `max-width` and horizontal padding matching `<main>`. An operator shapes it through the zone's `layoutConfig` — arrangement, gap, `collapseBelow`, and `customCss` — the same controls every other zone exposes.
...
**Widget types** are built by `buildCoreWidgetTypeDescriptors(deps)` in `widget-types/core-widget-types.ts` — a factory, not a static array, because some fetchers need runtime dependencies. Eight ship today: `core:raw-html` (operator-authored HTML/text block, read from `instanceConfig`), `core:world-clocks` (configured time-zone row), `core:layout-group` (the structural container — see [Single-level grouping](#single-level-grouping)), `core:block-ticker` (the real-time blockchain status row), `core:network-activity` (the activity chart), `core:auth-button` (the site's sign-in / profile button), `core:site-logo` (the wordmark linking home, with editable text), and `core:main-menu` (the `main` navigation, with an alignment setting). The ticker fetcher resolves the `'blockchain'` service from the registry at fetch time and returns `{ block }` — the latest processed block, or `null` when none is indexed (wrapped, never bare-null, so the resolver keeps the placement and the component still mounts to receive live `block:new` updates). raw-html, world-clocks, and layout-group ignore `deps`.

**The sign-in button is a widget, not part of the header.** Its fetcher reads the administrator's sign-in image from `ISystemConfigService` (injected into the module as `systemConfig`), falling back to `imageUrl: null` if the read fails so the site never loses its sign-in button. Whether the visitor is signed in is not in the payload: `AuthButtonWidget` reads it from the SSR-seeded session context, which keeps the payload the same for every visitor and so safe under the [route cache](#route-cache). A consequence is that a newly saved image can take up to five seconds to appear. Until an operator places the widget, typically in `site-top`, the site shows no sign-in button.

**Each placement can add a slide-out tray of links.** The placement settings are `links` (up to 12, each an `IAuthButtonLink` from the types package: `icon`, `label`, `url`, and `audience` of `everyone`, `signed-in`, or `signed-out`), `direction` (`right` or `down`), and `opacity` (40–100, the tray background's opacity in percent, default 85). The payload is `{ imageUrl, links, direction, opacity }`. The fetcher checks every link again and drops any whose URL is not a root-relative path or an absolute http(s) URL, whose icon cannot be a lucide name, or whose label is blank, so a hand-edited row costs one link rather than the button. The audience filter runs in the browser against the visitor's session, for the same route-cache reason as above. The frontend `AccountTray` (user module) takes over the button's click only when the visitor can see at least one link: a signed-in visitor then gets the tray instead of `/profile`, and a signed-out visitor gets the tray with a built-in Sign in entry first. With no visible links the click is unchanged. `right` falls back to `down` when the viewport has no room beside the button. The tray renders in a `<body>` portal because every zone is a size container, which also makes it a stacking context that later page content would paint over.

**The main menu's items never travel in widget data.** The `main` tree is filtered per visitor — admins see the System container — and the [route cache](#route-cache) shares one payload across visitors, so a tree in the payload would show one visitor's menu to the next. `core:main-menu`'s fetcher returns only `{ align }`. The root layout fetches the visitor's own tree on every request with their cookies (`fetchMenuNamespace` in the frontend menu module) and passes it through `MenuSeedProvider`; `MainMenuWidget` reads it with `useMenuSeed('main')`, and `MenuNavClient` seeds Redux from it as before. This is the pattern for any future widget whose content depends on the visitor: fetch it per request outside the widget payload and provide it to the widget, rather than weakening the cache.
...
`/system/widgets` is organised around a page. The operator picks a page from the `main` menu namespace (read server-side from the admin view, `GET /api/menu/manage`, so pages hidden from navigation are still offered while disabled ones are skipped), a route pattern a placement already uses, or a path they type; the board then shows every zone with the rows that resolve on that page, using a client-side mirror of `routeMatches`. "Every page" is the default and shows site-wide rows only. Rows placed in a zone that belong to other pages are never hidden: each zone lists them under a disclosure so an operator can always find a row they placed.
...
| `string` with `format: WIDGET_ICON_FORMAT.name` (`lucide-icon`) | Icon picker button; the placements API registers the format, so it also rejects a value that is not a PascalCase icon name |
