...
A child that carries a filter of its own keeps it. `findByRoute` applies the child's filter as well as the container's while the row is nested, so a child scoped `/markets/btc` inside a `/markets/**` container renders on one page only; overwriting it with the container's filter would widen the row rather than preserve it. The admin editor never creates that state — it clears `routes` on nesting — but a `PATCH` carrying `routes` without `parentId` does, since neither the parser nor the service clears them on a nested row. Intersecting the two filters is not an option: patterns are exact, trailing `/*`, or trailing `/**`, and an empty intersection cannot be written down, because `routes: []` already means every route.
