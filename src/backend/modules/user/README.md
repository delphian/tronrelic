...
**Bootstrap is Mongo-read-only; cookie-validated writes upsert via `ensureExists`.** Phase 2 of the traffic-events split (see [PLAN-traffic-events.md](../../../../PLAN-traffic-events.md)) made `POST /api/user/bootstrap` mint the cookie and emit one ClickHouse `traffic_event` without writing to the `users` collection. Phase 3 made `startSession` the first cookie-validated mutation that creates the row. Phase 4 generalised that pattern: every write endpoint that needs a persisted row routes through `UserService.ensureExists(userId)`, which upserts an anonymous document idempotently under concurrency. The cookie signature is the trust boundary — bots and uptime probes never get past `validateCookie`, so the orphan-row property holds even when a hand-rolled client targets `connectWallet`, `linkWallet`, `issueWalletChallenge`, or `updatePreferences` cold. Endpoints that *require an existing wallet* (`setPrimaryWallet`, `unlinkWallet`, `refreshWalletVerification`) deliberately skip the upsert and surface "User not found" because a missing row implies a missing wallet, and a synthesised row cannot satisfy the call. Observability writes (`recordActivity`, `recordPage`, `heartbeat`, `endSession`) silently no-op on a missing row so an idle tab cannot spawn rows. `logout` returns the same anonymous payload `bootstrap` produces and skips the write entirely — there is nothing to downgrade. `GET /api/user/:id` returns 404 when no row exists for the cookie-resolved UUID; `getServerUser` already maps 404 onto the unauthenticated shell, and `userContextMiddleware` proceeds with `req.user` undefined — both pre-existing graceful-degradation paths the orphan-row fix relies on.
