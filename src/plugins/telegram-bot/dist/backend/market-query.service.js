...
export class MarketQueryService {
    ...
    async fetchMarkets() {
        try {
            ...
            const response = await axios.get(`${this.apiBaseUrl}/plugins/resource-markets/markets`, {
                timeout: 5000
            });
     ...
     */
    findBestMarket(markets, targetMinutes) {
        let bestMarket = null;
        ...
        for (const market of markets) {
            ...
            const matchingCost = market.pricingDetail.usdtTransferCosts.find(c => c.durationMinutes === targetMinutes);
            if (matchingCost) {
                this.logger.debug({
                    marketName: market.name,
        ...
        if (bestMarket) {
            ...
            this.logger.warn({ targetMinutes }, 'No markets found for requested duration');
        }
...
//# sourceMappingURL=market-query.service.js.map