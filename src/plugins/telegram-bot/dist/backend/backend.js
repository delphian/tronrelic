...
export const telegramBotBackendPlugin = definePlugin({
    ...
    install: async (context) => {
        ...
        if (existingSubscriptions === 0) {
            ...
            await subscriptionsCollection.insertMany(defaultSubscriptions);
    ...
    init: async (context) => {
        ...
        if (!initialized) {
            ...
            context.logger.info({ maskedToken: maskedConfig.botToken }, 'Bot token loaded successfully from database');
        }
        ...
        const commandHandler = new CommandHandler(context.database, marketQueryService, context.logger);
        ...
        const webhookHandler = createWebhookHandler(commandHandler, telegramBotService, context, context.logger, {
        ...
        const webhookRouteHandler = async (req, res, _next) => {
            if (req.method === 'GET') {
        ...
        const webhookUrl = `${siteUrl}/api/plugins/telegram-bot/webhook`;
        context.logger.info({ webhookUrl, siteUrl }, 'Telegram webhook endpoint ready. Configure this URL in your Telegram bot settings.');
    ...
    adminRoutes: [
        ...
        },
        {
            ...
            handler: async (_req, res, next) => {
                try {
                    const usersCollection = pluginContext.database.getCollection('users');
                    // Calculate stats
                    ...
                    // Total commands
                    const commandsResult = await usersCollection.aggregate([
                        {
                            $group: {
                                _id: null,
                    ...
                    // Subscription counts
                    const subscriptionResult = await usersCollection.aggregate([
                        {
                    ...
                    // Get channel stats
                    const channelsCollection = pluginContext.database.getCollection('channels');
        ...
        },
        {
            ...
            handler: async (req, res, _next) => {
                try {
                    ...
                    const maskedConfig = await telegramBotService.getMaskedConfig();
                    pluginContext.logger.info({ updates: Object.keys(updates) }, 'Bot settings updated successfully');
                    res.json({
                        success: true,
