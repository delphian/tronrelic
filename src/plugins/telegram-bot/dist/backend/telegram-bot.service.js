...
export class TelegramBotService {
    ...
    async loadConfig() {
        ...
        // Load from database
        const dbConfig = await this.database.get('bot-config');
    ...
    async saveConfig(config) {
        ...
        const existingConfig = await this.database.get('bot-config') || {};
    ...
    validateBotToken(token) {
        ...
        if (!tokenPattern.test(token)) {
            throw new Error('Invalid bot token format. Expected format: <bot-id>:<random-token> (e.g., 123456789:ABCdefGHIjklMNOpqrsTUVwxyz)');
        }
    ...
    async sendNotification(telegramUserId, message) {
        if (!this.client) {
            throw new Error('Telegram client not initialized. Please configure bot token first.');
        }
        ...
        this.logger.info({ telegramUserId, messageLength: message.length }, 'TelegramBotService.sendNotification called (user lookup not yet implemented)');
    }
    async isSubscribed(telegramUserId, subscriptionType) {
        // TODO: Implementation requires:
        // 1. Look up user in database
        // 2. Check if subscriptionType is in user.subscriptions array
        // 3. Return true/false
        // 4. Handle case where user not found (return false)
        this.logger.info({ telegramUserId, subscriptionType }, 'TelegramBotService.isSubscribed called (not yet implemented)');
        return false;
    }
}
