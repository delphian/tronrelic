 ...
 */
async function trackChannelMembership(update, context, logger) {
    ...
    try {
        ...
        if (existingChannel) {
            ...
            await channelsCollection.updateOne({ chatId: chat.id }, {
                $set: {
                    type: chat.type,
                    title: chat.title,
        ...
        logger.error({ error, chatId: chat.id, title: chat.title }, 'Failed to track channel membership');
    }
}
/**
 ...
 */
async function trackChannelFromMessage(update, context, logger) {
    if (!update.message) {
        return;
    }
    ...
    try {
        ...
        logger.error({ error, chatId: chat.id, title: chat.title }, 'Failed to track channel from message');
    }
}
/**
 ...
 */
export function createWebhookHandler(commandHandler, telegramBotService, context, logger, securityOptions) {
    return async (req, res) => {
        ...
        try {
            ...
            logger.trace({ fullUpdate: update }, 'Full Telegram webhook payload');
            ...
            try {
                ...
                if (chatId) {
                    await telegramBotService.sendMessage(String(chatId), '⚠️ Sorry, something went wrong processing your request. Please try again later.', { parseMode: null });
