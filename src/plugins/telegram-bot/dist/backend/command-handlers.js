...
export class CommandHandler {
     ...
     */
    constructor(database, marketQueryService, logger) {
    ...
    async handleStart(update) {
        ...
        await this.trackUser(message.from.id, message.from?.username, message.from?.first_name, message.from?.last_name);
...
Stay tuned!`;
     ...
     */
    async handleUnsubscribe(update) {
        const message = update.message;
        const chatId = String(message.chat.id);
        await this.trackUser(message.from.id, message.from?.username, message.from?.first_name, message.from?.last_name);
