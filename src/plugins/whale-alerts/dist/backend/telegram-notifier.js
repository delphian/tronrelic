...
export class TelegramNotifier {
    ...
    logger;
    telegramToken;
    constructor(database, logger, telegramToken) {
    ...
    async sendPendingNotifications() {
        ...
        const unnotified = await this.database.find('transactions', { notifiedAt: null }, {
            sort: { timestamp: 1 },
            limit: 10
        });
        ...
        for (const whale of unnotified) {
            try {
                ...
                // Mark as notified
                await this.database.updateMany('transactions', { txId: whale.txId }, { $set: { notifiedAt: now } });
