...
export const whaleAlertsBackendPlugin = definePlugin({
    ...
    disable: async (context) => {
        ...
        const interval = global.__whaleAlertsTelegramInterval;
        if (interval) {
            clearInterval(interval);
            delete global.__whaleAlertsTelegramInterval;
    ...
    init: async (context) => {
        ...
        context.websocket.onSubscribe(async (socket, roomName, payload) => {
            ...
            context.logger.debug({ socketId: socket.id, roomName, payload }, 'Client subscribed to whale alerts');
        ...
        context.websocket.onUnsubscribe(async (socket, roomName, payload) => {
            ...
            context.logger.debug({ socketId: socket.id, roomName }, 'Client unsubscribed from whale alerts');
        });
        ...
        createWhaleDetectionObserver(context.BaseObserver, context.observerRegistry, context.websocket, context.database, context.logger);
        ...
        if (telegramToken) {
            context.logger.warn('DEPRECATED: whale-alerts uses legacy Telegram integration. ' +
            ...
            global.__whaleAlertsTelegramInterval = notificationInterval;
        }
        else {
    ...
    routes: [
        {
            ...
            handler: async (req, res, next) => {
                try {
                    ...
                    const transactions = await pluginContext.database.find('transactions', {}, {
                        sort: { timestamp: -1 },
                        limit
                    });
                    const highlights = transactions.map(tx => ({
                        txId: tx.txId,
                        timestamp: tx.timestamp,
                        amountTRX: tx.amountTRX,
                        fromAddress: tx.fromAddress,
