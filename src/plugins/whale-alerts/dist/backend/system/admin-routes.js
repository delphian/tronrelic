        ...
        },
        {
            ...
            handler: async (req, res, next) => {
                try {
                    ...
                    context.logger.info({ config }, 'Whale alerts configuration updated');
                    ...
                    context.logger.debug({ thresholdTRX: config.thresholdTRX }, 'Broadcasted config update to all clients');
                    res.json({ success: true, config });
                }
                catch (error) {
