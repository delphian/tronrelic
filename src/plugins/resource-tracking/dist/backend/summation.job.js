 ...
 */
async function initializeAggregationState(database, logger) {
    // Find the earliest block in transactions collection
    const earliestTxs = await database.find('transactions', {}, { sort: { blockNumber: 1 }, limit: 1 });
    ...
    logger.info({ initialBlock, lastProcessedBlock: state.lastProcessedBlock }, 'Initialized block aggregation state');
    return state;
}
/**
 ...
 */
async function detectAndSkipGap(database, logger, currentState, blocksPerInterval) {
    // Find earliest available transaction
    const earliestAvailableTxs = await database.find('transactions', {}, { sort: { blockNumber: 1 }, limit: 1 });
 ...
 */
export async function runSummationJob(database, logger, websocket) {
    const jobStartTime = new Date();
    const MAX_TRANCHES_PER_RUN = 4;
    try {
        // Step 1: Load or initialize aggregation state
        let state = await database.get('aggregation-state');
        if (!state) {
        ...
        for (let tranche = 0; tranche < MAX_TRANCHES_PER_RUN; tranche++) {
            ...
            state = await database.get('aggregation-state');
            if (!state) {
            ...
            const latestTxs = await database.find('transactions', {}, { sort: { blockNumber: -1 }, limit: 1 });
