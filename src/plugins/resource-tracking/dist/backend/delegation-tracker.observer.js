 ...
 */
export function createDelegationTrackerObserver(BaseObserver, observerRegistry, database, logger) {
    ...
    class DelegationTrackerObserver extends BaseObserver {
        name = 'DelegationTrackerObserver';
        database;
        config = null;
        configLastLoaded = 0;
        CONFIG_CACHE_MS = 5 * 60 * 1000; // 5 minutes
        constructor() {
            super(scopedLogger);
        ...
        async process(transaction) {
            ...
            const contractParams = payload.contract?.parameters;
            ...
            const resourceType = (typeof resourceValue === 'string' && resourceValue.toUpperCase() === 'ENERGY') ||
         ...
         */
        async loadConfigIfStale() {
            const now = Date.now();
            if (!this.config || (now - this.configLastLoaded) > this.CONFIG_CACHE_MS) {
                this.config = await this.database.get('config') ?? null;
         ...
         */
        async detectWhale(delegation, logger) {
            try {
