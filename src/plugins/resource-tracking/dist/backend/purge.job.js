 ...
 */
export async function runPurgeJob(database, logger) {
    ...
    try {
        ...
        const transactionsDeleted = await database.deleteMany('transactions', { timestamp: { $lt: transactionCutoff } });
        logger.info({
        ...
        const summationsDeleted = await database.deleteMany('summations', { timestamp: { $lt: summationCutoff } });
        logger.info({
