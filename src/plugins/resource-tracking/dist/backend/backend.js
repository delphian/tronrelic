...
export const resourceTrackingBackendPlugin = definePlugin({
    ...
    init: async (context) => {
        ...
        const observer = createDelegationTrackerObserver(context.BaseObserver, context.observerRegistry, context.database, context.logger);
    ...
    routes: [
        {
            ...
            handler: async (req, res, next) => {
                try {
                    ...
                    const cached = await pluginContext.cache.get(cacheKey);
                    if (cached) {
                        pluginContext.logger.debug({ period, points: requestedPoints, cacheKey }, 'Summation cache hit');
                        return res.json(cached);
                    }
                    pluginContext.logger.debug({ period, points: requestedPoints, cacheKey }, 'Summation cache miss - fetching from database');
                    ...
                    const summations = await pluginContext.database.find('summations', { timestamp: { $gte: startDate } }, { sort: { startBlock: 1 } });
                    ...
                    const response = {
                        success: true,
                        data: formattedData, // Formatted for frontend (Date → ISO string, SUN → TRX)
        ...
        },
        {
            ...
            handler: async (req, res, next) => {
                try {
                    const { detailsRetentionDays, summationRetentionMonths, purgeFrequencyHours, blocksPerInterval, whaleDetectionEnabled, whaleThresholdTrx } = req.body;
                    ...
                    if (purgeInterval) {
                        ...
                        pluginContext.logger.info({ intervalHours: config.purgeFrequencyHours }, 'Purge job restarted with new frequency');
                    }
                    res.json({ success: true, settings: config });
                }
                catch (error) {
        ...
        },
        {
            ...
            handler: async (req, res, next) => {
                try {
                    ...
                    const whales = await pluginContext.database.find('whale-delegations', filter, {
                        sort: { timestamp: -1 },
                        limit
                    });
    ...
    adminRoutes: [
        {
            ...
            handler: async (req, res, next) => {
                try {
                    ...
                    await Promise.all(keys.map((key) => pluginContext.cache.del(key)));
                    pluginContext.logger.info({ keysCleared: keys.length, pattern }, 'Summation cache cleared successfully');
                    res.json({
                        success: true,
